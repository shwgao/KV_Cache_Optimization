...
OUT_DIR=${OUT_DIR:-../../analysis/results/complete_rag_analysis}
...
echo "Running complete RAG analysis..."

python3 ../../analysis/rag_seqlen_distribution.py \
    --inputs "$CHUNKS_JSON" \
    --text-fields query,context,answer \
    --tokenizer "$MODEL" \
    --max-samples 5000 \
    --out-csv "$OUT_DIR/seq_lengths.csv" \
    --out-plot "$OUT_DIR/seq_distribution.png"
...
python3 ../../analysis/rag_chunk_sparsity.py \
    ...
    --max-new-tokens "$MAX_NEW_TOKENS" \
    --attention-threshold "$ATTENTION_THRESHOLD" \
    --out-dir "$OUT_DIR/sparsity" \
    --query "$QUERY" \
    $CHUNKS_ARG

python3 ../../analysis/rag_chunk_temporal_analysis.py \
    --input-json "$OUT_DIR/sparsity/sparsity_analysis.json" \
    --output-dir "$OUT_DIR/temporal" \
    --top-k-chunks 5 \
    --intervals "1,5,10,25,50,100"

echo "Complete RAG analysis finished. Results saved to: $OUT_DIR"
