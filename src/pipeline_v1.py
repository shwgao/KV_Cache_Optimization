...
def _load_and_merge_kvs(
    ...
    Expects per-chunk folders: {sample_dir}/{idx}/keys.pt, values.pt
    ...
    def _normalize_kv(k: torch.Tensor, v: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # Accepts [L,H,S,D] or [L,B,H,S,D] -> returns [L,1,H,S,D]
        if k.ndim == 4:
            k = k.unsqueeze(1)
        if v.ndim == 4:
            v = v.unsqueeze(1)
...
class FinalDecoder:
    ...
    def _stream_generate(self, gen_kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Metrics (end-to-end based throughput/TPOT)
        num_toks = len(pieces)
        e2e = t1 - t0
    ...
    def decode_with_saved_kvs(
        ...
        # If no KVs available, just decode with question suffix only (degenerate case)
        suffix = f"Question: {sample.get('question','')}\nAnswer:"
        enc = self.tok([suffix], return_tensors="pt", padding=False, truncation=False)
        input_ids = enc["input_ids"].to(self.device)

        # Always pass attention_mask to avoid warnings; for single sequence of len L it's ones
        ...
        if past is not None and len(past) > 0 and past[0][0].shape[2] > 0:
            cached_len = past[0][0].shape[2]  # [1,H,S_cached,D]
            # Expand attention over cached + new prompt tokens
            total_len = cached_len + input_ids.shape[1]
            attn_mask_full = torch.ones((1, total_len), dtype=torch.long, device=self.device)
            # position ids are optional for Llama; cache_position is important for HF>=4.40
...
def main():
    ap = argparse.ArgumentParser("Full KV Reuse Pipeline (simplified)")
    ...
    for si in tqdm(range(len(samples)), desc="Samples", unit="sample"):
        ...
        # For this simplified pipeline, just use all GPU-placed chunks (0..top_k-1 if present)
        # If your builder stores placements, you can read and pick exactly the GPU ones.
        gpu_indices = []
        for name in os.listdir(sample_dir):
            if name.isdigit():
                gpu_indices.append(int(name))
        gpu_indices = sorted(gpu_indices)

        # 3) Final decoding with saved KVs
        ...
        # 4) Flush rolling final results + summary
